...
export default function HybridAuthPage() {
  ...
  // Handler for both Login and Signup (Step 1: Request OTP)
  const handleProviderAuthRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      if (providerMode === "signup") {
        ...
        if (ProviderverficationOTP.success) {
          toast({ title: `${ProviderverficationOTP.message}` });
          setProviderStep(2);
        } else {
          toast({ title: `${ProviderverficationOTP.message}`, variant: "destructive" });
        ...
        if (ProviderverficationOTPLogin.success) {
          toast({ title: `${ProviderverficationOTPLogin.message}` });
          setProviderStep(2);
        } else {
    ...
    try {
      if (providerMode === "signup") {
        ...
        if (Providerverfication.success) {
          ...
          toast({ title: `${Providerverfication.message}`, variant: "destructive" });
        }
      } else {
        if (providerOtp.length !== 4) {
          toast({ title: "Enter 4 digit OTP" });
        ...
        if (ProviderverficationLogin.success) {
          ...
          toast({ title: `${ProviderverficationLogin.message}`, variant: "destructive" });
        }
      }
    } catch (error) {
      console.log(error);
          ...
          <CardContent className="pt-10 pb-10 px-10">
            ...
            <div className="flex justify-center mb-8">
              <div className="inline-flex bg-slate-100 p-1 rounded-full">
                <button
                  ...
                  className={`px-6 py-2 rounded-full text-xs font-bold transition-all ${(role === "user" ? userMode === "signup" : providerMode === "signup")
                ...
                <button
                  ...
                  className={`px-6 py-2 rounded-full text-xs font-bold transition-all ${(role === "user" ? userMode === "login" : providerMode === "login")
                    ? "bg-white shadow-sm text-slate-900" : "text-slate-400"
                    }`}
                >LOGIN</button>
              </div>
            ...
            {role === "provider" && (
              <div className="space-y-6 animate-in fade-in duration-300">
                {providerStep === 1 && (
                  <form onSubmit={handleProviderAuthRequest} className="space-y-5">
                    {providerMode === "signup" && (
                      <div className="relative group">
                    ...
                    <Button disabled={isLoading} className="w-full h-14 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl font-bold shadow-xl transition-all">
