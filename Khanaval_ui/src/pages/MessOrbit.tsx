          ...
          <div className="flex bg-slate-100 p-1 rounded-2xl gap-1 overflow-x-auto no-scrollbar">
            {['nearby', 'veg', 'price'].map((tab) => (
              <button
                ...
                className={`whitespace-nowrap px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${
    ...
    <div key={mess.id} className="group relative bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_15px_40px_rgba(0,0,0,0.04)] overflow-hidden active:scale-[0.98] transition-all">
      ...
      <div className="p-6 pt-4">
        ...
        <div className="flex items-center gap-3">
          <button className="flex-1 py-3.5 bg-slate-900 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-2 group-hover:bg-orange-600 transition-all shadow-lg shadow-slate-200">
            View Menu <ArrowRight className="w-4 h-4" />
          </button>
          
          <div className={`px-4 py-3 rounded-2xl border-2 flex items-center justify-center ${
            mess.identity?.dietaryType === 'Pure Veg' 
