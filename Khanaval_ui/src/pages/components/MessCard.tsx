...
const MessCard = ({ _id, identity, media, MontlyPrices, rating, location, messVerified, legal, providerId }) => {
    ...
    // Price Check Logic
    const hasMonthlyPrice = identity?.MonthlyPrice && Number(identity?.MonthlyPrice) > 0;
                            ...
                            {MontlyPrices ? (
                                ...
                                <>
                                    <div className="flex flex-col min-w-[65px]">
                                        ...
                                        <span className="text-sm font-black text-slate-800">₹{MontlyPrices}</span>
                                        <button
                                ...
                                <>
                                    <Button
                                        variant="outline"
                                        onClick={() => user ? setIsRating(true) : navigate(`/auth`)}
                                        className="flex-1 rounded-xl border-slate-200 h-10 font-black text-[10px] text-slate-600 hover:bg-slate-50 transition-all flex gap-1.5"
                                    >
                                        <MessageSquare className="w-3 h-3" /> FEEDBACK
                                    </Button>
                                    <Button
                                        onClick={() => navigate(user || Providerdata ? `/mess/${_id}` : `/auth`)}
                                        className="flex-1 rounded-xl bg-slate-900 h-10 font-black text-[10px] text-white hover:bg-orange-500 transition-all shadow-lg"
                                    >
                                        VIEW MENU
                                    </Button>
                                </>
